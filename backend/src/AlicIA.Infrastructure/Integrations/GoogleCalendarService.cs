...
public class GoogleCalendarService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using var request = new HttpRequestMessage(
            HttpMethod.Post,
            $"https://www.googleapis.com/calendar/v3/calendars/{Uri.EscapeDataString(GetCalendarId(connection))}/events");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using var request = new HttpRequestMessage(
            HttpMethod.Put,
            $"https://www.googleapis.com/calendar/v3/calendars/{Uri.EscapeDataString(GetCalendarId(connection))}/events/{Uri.EscapeDataString(eventId)}");
        ...
        if (!response.IsSuccessStatusCode)
            throw new InvalidOperationException($"Failed to update Google Calendar event: {content}");

        using var doc = JsonDocument.Parse(content);
        var updatedEventId = doc.RootElement.GetProperty("id").GetString();

        if (string.IsNullOrWhiteSpace(updatedEventId))
            throw new InvalidOperationException("Google Calendar did not return an event id.");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        throw new InvalidOperationException($"Failed to delete Google Calendar event: {content}");
    }

    private static string GetCalendarId(CalendarConnection connection)
    {
        return string.IsNullOrWhiteSpace(connection.CalendarId) ? "primary" : connection.CalendarId;
    }
}
