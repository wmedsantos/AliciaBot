...
app.MapPost("/api/requests/{requestId:guid}/cancel", async (
    Guid requestId,
    HttpContext httpContext,
    GoogleCalendarService googleCalendarService,
    AlicIADbContext db,
    CancellationToken cancellationToken) =>
{
    ...
    if (request.Status is RequestStatus.Completed or RequestStatus.NoShow)
        return Results.BadRequest(new { error = "Completed or no-show requests cannot be cancelled." });

    if (request.Status == RequestStatus.Cancelled)
        return Results.BadRequest(new { error = "Request is already cancelled." });
    ...
    if (!string.IsNullOrWhiteSpace(request.ExternalEventId))
    {
        var connection = await db.CalendarConnections
            .FirstOrDefaultAsync(
                x => x.TenantId == tenantId.Value && x.Provider == "Google" && x.IsActive,
                cancellationToken);

        if (connection is null)
            return Results.BadRequest(new { error = "Active Google Calendar connection not found for this tenant." });

        calendarEventDeleted = await googleCalendarService.DeleteEventAsync(
            connection,
            request.ExternalEventId,
            cancellationToken);

        request.ExternalEventId = null;
    }
...
app.MapPost("/api/requests/{requestId:guid}/reschedule", async (
    Guid requestId,
    ...
    CancellationToken cancellationToken) =>
{
    ...
    var bookingRequest = await db.Requests
        .Include(x => x.Customer)
        .Include(x => x.Service)
        .FirstOrDefaultAsync(x => x.Id == requestId && x.TenantId == tenantId.Value, cancellationToken);
    ...
    if (bookingRequest.Status is RequestStatus.Completed or RequestStatus.NoShow or RequestStatus.Cancelled)
        return Results.BadRequest(new { error = "Request cannot be rescheduled in its current status." });

    var service = bookingRequest.Service;
    if (service is null)
        return Results.BadRequest(new { error = "Service not found for request." });

    var scheduledAtUtc = request.ScheduledAt.ToUniversalTime();
    var availability = await ValidateSlotAvailableAsync(
        db,
        ...
        excludedRequestId: bookingRequest.Id,
        cancellationToken);

    if (!availability.IsAvailable)
        return Results.BadRequest(new { error = availability.Error });

    if (availability.Connection is null)
        return Results.BadRequest(new { error = "Active Google Calendar connection not found for this tenant." });

    var description = $"Customer: {bookingRequest.Customer!.Name} | Phone: {bookingRequest.Customer.Phone}";
    if (string.IsNullOrWhiteSpace(bookingRequest.ExternalEventId))
    {
        bookingRequest.ExternalEventId = await googleCalendarService.CreateEventAsync(
    ...
    {
        bookingRequest.ExternalEventId = await googleCalendarService.UpdateEventAsync(
            availability.Connection,
            bookingRequest.ExternalEventId,
            service.Name,
            scheduledAtUtc,
            availability.EndUtc,
            description,
    ...
    bookingRequest.ScheduledAt = scheduledAtUtc;
    bookingRequest.Status = RequestStatus.Rescheduled;
    await db.SaveChangesAsync(cancellationToken);
...
app.MapPost("/public/{tenantSlug}/bookings", async (
                ...
                CancellationToken cancellationToken) =>
            {
                ...
                bookingRequest.ConfirmationTokenHash = HashConfirmationToken(confirmationToken);
                ...
                return Results.Created("/public/bookings", new
                {
                    status = "confirmed",
...
app.MapPost("/public/bookings/{confirmationToken}/cancel", async (
    ...
    CancellationToken cancellationToken) =>
{
    if (string.IsNullOrWhiteSpace(confirmationToken))
        return Results.BadRequest(new { error = "Confirmation token is required." });

    var tokenHash = HashConfirmationToken(confirmationToken);
    var bookingRequest = await db.Requests
        .FirstOrDefaultAsync(x => x.ConfirmationTokenHash == tokenHash, cancellationToken);

    if (bookingRequest is null)
        return Results.NotFound(new { error = "Booking not found." });

    if (bookingRequest.Status is RequestStatus.Completed or RequestStatus.NoShow)
        return Results.BadRequest(new { error = "Completed or no-show bookings cannot be cancelled." });
    ...
    if (!string.IsNullOrWhiteSpace(bookingRequest.ExternalEventId))
    {
        var connection = await db.CalendarConnections
            .FirstOrDefaultAsync(
                x => x.TenantId == bookingRequest.TenantId && x.Provider == "Google" && x.IsActive,
                cancellationToken);
        ...
        bookingRequest.ExternalEventId = null;
    }
...
app.MapPost("/public/bookings/{confirmationToken}/reschedule", async (
    string confirmationToken,
    RescheduleRequest request,
    GoogleCalendarService googleCalendarService,
    AlicIADbContext db,
    CancellationToken cancellationToken) =>
{
    ...
    if (bookingRequest.Status is RequestStatus.Completed or RequestStatus.NoShow or RequestStatus.Cancelled)
        return Results.BadRequest(new { error = "Booking cannot be rescheduled in its current status." });

    var service = bookingRequest.Service;
    if (service is null)
        return Results.BadRequest(new { error = "Service not found for booking." });
...
static async Task<SlotAvailabilityResult> ValidateSlotAvailableAsync(
    AlicIADbContext db,
    GoogleCalendarService googleCalendarService,
    Guid tenantId,
    Service service,
    DateTime scheduledAtUtc,
    bool requireGoogleConnection,
    ...
    CancellationToken cancellationToken)
{
    ...
    var businessHours = await db.BusinessHours
        .Where(x => x.TenantId == tenantId && x.IsActive && x.DayOfWeek == scheduledAtUtc.DayOfWeek)
        .ToListAsync(cancellationToken);

    if (!businessHours.Any())
        return SlotAvailabilityResult.Unavailable("No active business hours found for the requested day.", scheduledEndUtc);

    var validPeriod = businessHours.Any(x =>
        scheduledAtUtc.TimeOfDay >= x.StartTime && scheduledEndUtc.TimeOfDay <= x.EndTime);

    if (!validPeriod)
        return SlotAvailabilityResult.Unavailable("Requested slot is outside of business hours.", scheduledEndUtc);
    ...
    var existingRequests = await db.Requests
        ...
        .Where(x =>
            ...
            x.ScheduledAt != null &&
            x.Status != RequestStatus.Cancelled &&
            (!excludedRequestId.HasValue || x.Id != excludedRequestId.Value))
        .ToListAsync(cancellationToken);

    if (existingRequests.Any(x =>
            BookingHelpers.Overlaps(
                scheduledAtUtc,
                scheduledEndUtc,
                x.ScheduledAt!.Value.ToUniversalTime(),
                x.ScheduledAt.Value.ToUniversalTime().AddMinutes(x.Service?.DurationMinutes ?? 0))))
    {
        return SlotAvailabilityResult.Unavailable("Requested slot is already occupied by another booking.", scheduledEndUtc);
    }
    ...
    if (connection is null)
    {
        return requireGoogleConnection
            ? SlotAvailabilityResult.Unavailable("Active Google Calendar connection not found for this tenant.", scheduledEndUtc)
            : SlotAvailabilityResult.Available(scheduledEndUtc, null);
    }
    ...
    if (googleBusySlots.Any(x => BookingHelpers.Overlaps(scheduledAtUtc, scheduledEndUtc, x.StartUtc, x.EndUtc)))
        return SlotAvailabilityResult.Unavailable("Requested slot is busy in Google Calendar.", scheduledEndUtc);

    return SlotAvailabilityResult.Available(scheduledEndUtc, connection);
}
...
static string HashConfirmationToken(string confirmationToken)
{
    var normalizedToken = confirmationToken.Trim();
...
{
    public static SlotAvailabilityResult Available(DateTime endUtc, CalendarConnection? connection)
    {
        return new SlotAvailabilityResult(true, null, endUtc, connection);
    }

    public static SlotAvailabilityResult Unavailable(string error, DateTime endUtc)
    {
        return new SlotAvailabilityResult(false, error, endUtc, null);
    }
}
